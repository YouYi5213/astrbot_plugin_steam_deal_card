...
- **限时免费领取（喜加一）**：列出当前可以**免费永久入库**的游戏，含封面、原价、
  好评率和**领取截止时间**。错过就没了，所以截止时间是卡片上最显眼的信息。
...
- `steam免费`：`steam限免`、`steam喜加一`、`steam限时免费`
...
| 限时免费领取（喜加一） | Steam 商店搜索 `specials=1&maxprice=free&category1=998` |
| 中文游戏名 → Steam appid | 小黑盒公开搜索接口 **＋** Steam 商店搜索接口 |
| 历史最低价 | 小黑盒公开历史价格接口 |
...
限时免费**不是**「-100% 的折扣」，Steam 用的是另一套字段，实测：

| | 普通打折 | 限时免费领取 |
| --- | --- | --- |
| 标识 | `discount_pct` = 90 | `is_free_to_keep` = `true` |
| 截止时间 | `active_discounts[].discount_end_date` | **`free_to_keep_ends`** |
| `active_discounts` | 有值 | **是 `null`** |

所以只读折扣字段的话，领取截止时间会是**空的** —— 而截止时间恰恰是这个功能
最重要的信息。卡片上的措辞也跟着区分：打折说「折扣 09-24 结束」，
喜加一说「**领取截止** 09-23（剩 3 天）」，因为错过喜加一是**失去游戏**，
不是多花钱。

另外两个实测细节：

- `specials=1&maxprice=free` 出来的**都是真·喜加一**（实测 4 条全部原价非零、
  -100%），不需要再按原价二次过滤；
- 但这个列表**会混进 DLC**（实测混进过一个武器包），所以要加
  `category1=998` 限定本体 —— 加上之后正好把那个 DLC 过滤掉。

## 实现说明

- **中文名转换**：由**两个**接口共同完成，缺一不可。
  - 小黑盒搜索接口认得口语化的中文名（`泰拉瑞亚`、`只狼`），但**它会给没有
    Steam appid 的游戏发自己的合成 id**（`9xxxxxxxx`）。这类 id 在 Steam 上
    并不存在，因此会被丢弃：`崩坏3` 在 Steam 是 `1668940`，小黑盒给的是
    `900045980`。实测 137 条 PC 记录里，凡是带真实 `appid` 字段的全部有效，
    没有该字段的 30 条全是合成 id（抽样 12 条无一存在）。
  - Steam 商店搜索接口只认**官方中文标题**：实测 `崩坏3`、`巫师3`、
    `黑神话：悟空`、`艾尔登法环`、`赛博朋克2077`、`双人成行` 都能命中，
    而 `泰拉瑞亚`、`只狼`、`空洞骑士` 这类简称返回 0 条。
  两个接口各能解决对方解决不了的名字，所以结果会**合并后统一打分**，
  因此 `崩坏3` 这类游戏也能查到。整个过程不需要 LLM，也不依赖任何密钥。
- **搜索接口有站点兜底**：`/api/storesearch/` 只有国际站提供，而大陆服务器到
  国际站可能整段时间不可达。此时会回退到 `/search/results/`，该接口**两个站都
  提供**，因此国际站挂掉时仍能解析中文名（实测国内站可返回 `崩坏3` → `1668940`）。
  接口正常返回空结果才算「没有这个游戏」，与「接口不通」区分开。
- **名称打分**：拿到候选后按「精确 100 / 前缀 85 / 子串 70 / 按序跳字 60 /
  分词重叠 40+」打分，取最高分。
  「按序跳字」这一档是给**标题中间插字**的情况用的：中文没有词边界，
  官方名 `荒野大镖客：救赎2` 比口语简称 `荒野大镖客2` 多了「救赎」两字，
  前缀和子串都匹配不上。为避免误伤，这一档要求短的一方含中文、长度 ≥ 3、
  且覆盖长的一方一半以上，因此 `幻塔` 不会匹配到无关的长标题，
  `gtav` 也不会匹配 `Grand Theft Auto V`。
...
- **在线人数与热度榜**：Steam 的热门榜接口返回的是 `peak_in_game`，它是
  **上一个完整自然日的峰值，不是当天、也不是实时人数**。接口同时返回
  `rollup_date` 指明是哪一天：实测 2026-09-20 14:30 UTC 抓取时该字段为
  `2026-09-19`，即 38.5 小时前的那个自然日。（Steam 网页版另有一列
  「今日峰值 / Peak Today」，但那是另一个数据源，这个接口拿不到。）
  所以卡片上会**带上日期**显示成「Steam 峰值 131.8 万（09-19）」，
  避免被读成今天的数据。
  峰值与实时人数的排序差异也很大（`GTA V` 峰值第 8、实时第 34；
  `Rust` 峰值第 26、实时第 13），比值在 0.26 ~ 1.01 之间浮动，无法换算。
  因此榜单**只用来挑选候选游戏**，排序一律用 `GetNumberOfCurrentPlayers`
  的实时值逐游戏查询后重新排序；峰值仅作参考信息单独标注，绝不当作实时人数展示。
...
- `steam免费` 的数据只有**全球商店**有：蒸汽平台（`store.steamchina.com`）
  实测对该查询返回空。所以当全球商店不可达时，命令不会说「没有喜加一」
  （v1.5.1 修），而是改用**上次成功抓取的缓存**并在卡片上标注缓存时间
  （v1.5.2）。缓存里截止时间已过的条目会被丢弃，因为那些已经领不到了。
