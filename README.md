...
- **在线人数**：查询单个游戏的**实时**在线人数，或按实时人数从高到低查看热度排行，
  卡片带游戏封面缩略图。
- **热门新品**：Steam 商店首页「热门新品」货架，含封面、价格、评价与史低。
- **热门即将推出**：Steam 商店首页「热门即将推出」货架，含封面、价格与**发售日期**。
...
| 热门新品榜 | Steam 商店首页「热门新品」货架（内嵌于首页 HTML） |
| 热门即将推出榜 | Steam 商店首页「热门即将推出」货架（同一份首页 HTML） |
...
- `steam热门新品` 与 `steam热门即将推出` 取自 Steam 商店首页的两个货架，与页面
  完全一致。这两个榜**无法用搜索接口复现**：`newreleases`、`popularnew`、
  `Reviews_DESC` 等参数组合与货架的重合度均为 0，因此直接解析首页 HTML。
  货架数量有限（各约 30 条），命令的条数上限受此限制。
